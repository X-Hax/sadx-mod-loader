...
FunctionPointer(signed int, njCheckFileName, (char* destination, const char* source, const char* extension), 0x0077F470);
FunctionPointer(Uint8*, njOpenBinary, (const char* str), 0x007929D0);
FunctionPointer(Uint8*, njCloseBinary, (void* str), 0x00792A70);
...
unsigned short FlipEndian(unsigned short val)
{
	unsigned char* p = (unsigned char*)&val;
	unsigned char shortVal[2];
	shortVal[0] = p[1];
	shortVal[1] = p[0];
	return *(unsigned short*)shortVal;
}
unsigned int FlipEndianint(unsigned int val)
{
	unsigned char* p = (unsigned char*)&val;
	unsigned char shortVal[4];
	shortVal[0] = p[3];
	shortVal[1] = p[2];
	shortVal[2] = p[1];
	shortVal[3] = p[0];
...
NJS_TEXSURFACE* sub_4307A0(NJS_TEXMEMLIST* a1, int a2)
{
	...
	result->nWidth = (unsigned __int16)FlipEndian(*(_WORD*)(a2 + 0xC));
	result->nHeight = (unsigned __int16)FlipEndian(*(_WORD*)(a2 + 14));
	v7 = *(_DWORD*)(a2 + 4) - 8;
...
IDirect3DTexture8* __cdecl sub_433930(NJS_TEXMEMLIST* texture, int data, int pixelformat, D3DFORMAT dataformatmaybe)
{
	...
	if (!v4->texsurface.pSurface)
	{
		...
		if (width > 0 && height > 0)
		{
			...
			v21 = 1;
			//v19 = levelcount
			D3DXCreateTexture(*(LPDIRECT3DDEVICE8*)0x03D128B0, v15, height, 1, 0, dataformatmaybe, D3DPOOL_MANAGED, &d3dtex);
...
void* njLoadTextureTexMemList(void* pFile, int globalindex)
{
	signed int v3; // edi
...
signed int njLoadTextureGvmMemory(int* a1, NJS_TEXLIST* a2)
{
	...
	if (*v2 != 'TRVG')                        // 'GVRT'
	{
		...
		if (!v7 || v7 == 9)
		{
			*((Uint16*)v2 + 4) = FlipEndian(*((Uint16*)v2 + 4));
			v8 = FlipEndian(*((Uint16*)v2 + 5));
			*((Uint16*)v2 + 5) = v8;
			testtest = v2;
			a2->nbTexture = v8;
		}
...
LABEL_11:
	...
	v10 = (NJS_TEXMEMLIST*)njLoadTextureTexMemList(v2, v9);
	v11 = v4++;
...
Sint32 __cdecl njLoadTextureGvmFile(const char* filename, NJS_TEXLIST* texList)
{
	Sint32 result; // eax MAPDST
	Uint8* data; // esi
	char name[256]; // [esp+4h] [ebp-104h]

	if (filename == 0 || texList == 0)
	{
		return -1;
	}
	njCheckFileName(name, filename, ".gvm");
	data = njOpenBinary(name);
